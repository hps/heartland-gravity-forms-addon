    ...
    private function authorizeACH($feed, $submission_data, $form, $entry) {
        ...
        try {
            $response = $service
                ...
                ->withCheck($check)
                /**@throws HpsCheckException on error */
                ->execute();
            $type = 'Payment';
            $amount_formatted = GFCommon::to_money($submission_data['payment_amount'], GFCommon::get_currency());
            $note = sprintf(__('%s has been completed. Amount: %s. Transaction Id: %s.', $this->_slug), $type, $amount_formatted, $response->transactionId);
