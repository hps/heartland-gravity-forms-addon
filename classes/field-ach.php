...
class GF_Field_HPSach extends GF_Field {
	...
	public function get_field_input($form, $value = '', $entry = null ) {
		...
		if ( is_array( $value ) ) {
			...
			$account_type = rgget( $this->id . '.2', $value );
...
//customer name
		  ...
		  $account_name_label       		= rgar( $account_name_field_input, 'customLabel' ) != '' ? $account_name_field_input['customLabel'] : esc_html__( 'Accountholder Name', 'gravityforms' );
...
//account number
			...
			$account_number_label       	= rgar( $card_number_field_input, 'customLabel' ) != '' ? $card_number_field_input['customLabel'] : esc_html__( 'Card Number', 'gravityforms' );
			$account_number_label       	= gf_apply_filters( array( 'gform_card_number', $form_id ), $card_number_label, $form_id );

//routing number
			$routing_number_field_input 	= GFFormsModel::get_input( $this, $this->id . '.1' );
			$html5_output            			= ! is_admin() && GFFormsModel::is_html5_enabled() ? "pattern='[0-9]*' title='" . esc_attr__( 'Only digits are allowed', 'gravityforms' ) . "'" : '';
			$routing_number_label         = rgar( $card_number_field_input, 'customLabel' ) != '' ? $card_number_field_input['customLabel'] : esc_html__( 'Card Number', 'gravityforms' );
			$routing_number_label       	= gf_apply_filters( array( 'gform_card_number', $form_id ), $card_number_label, $form_id );

//account type
			$account_type_input           = GFFormsModel::get_input( $this, $this->id . '.2_month' );
			$account_type                 = $this->get_expiration_months( $expiration_month );
